...
const M3_DIAGNOSTIC_ITEMS: Record<string, UnknownRecord[]> = {
  ...
  M3_L6: [
    ...
    shortItem("M3L6_D7", "What should you decide before choosing equations in a long energy mission?", ["which store or hand-off each step describes", "the mission stages and ledger changes", "what changes, what leaks, and what the target is", "which result from one step becomes the input to the next", "what output or bridge quantity each stage provides"], "Use store-change planning language, not just 'pick a formula'.", phraseGroups(["step", "stage", "sequence", "order", "plan"], ["store", "transfer", "hand-off", "leak", "target", "result", "output", "input", "quantity", "bridge"])),
...
const M3_CONCEPT_ITEMS: Record<string, UnknownRecord[]> = {
  ...
  M3_L5: [
    ...
    shortItem("M3L5_C5", "In a few words, what does efficiency compare?", ["useful output to total input", "useful energy over total energy", "useful gain divided by total input", "useful output divided by input"], "State it as useful part compared with total input.", phraseGroups(["useful", "useful output", "useful energy"], ["input", "total", "total input"], ["divide", "divided", "fraction", "proportion", "percentage", "compared", "over"])),
  ...
  M3_L6: [
    ...
    shortItem("M3L6_C5", "What is the first question to ask in a long energy mission?", ["what changes store first", "which step happens first", "what quantity is created before the next step", "what the first ledger step is", "which result from one step becomes the input to the next", "what bridge quantity the next stage needs"], "Use sequence and store-change language.", phraseGroups(["first", "step", "stage", "sequence", "order", "plan"], ["store", "change", "created", "gain", "quantity", "target", "leak", "result", "output", "input", "bridge"])),
...
const M3_CONCEPT_EXPANSIONS: Record<string, UnknownRecord[]> = {
  ...
  M3_L6: [
    ...
    shortItem("M3L6_C8", "Why is backward planning sometimes helpful in long missions?", ["because the final target can tell you the earlier required input", "because it traces the target back through yields or leaks", "because you can work back from the goal to the earlier required values", "because the final requirement tells you what earlier store or input is needed"], "Use target-backward language.", phraseGroups(["target", "final", "goal", "requirement", "required"], ["back", "backward", "trace", "work back", "reverse"], ["input", "required", "earlier", "needed", "store", "stage", "step", "quantity", "value"])),
  ],
};
...
const M3_MASTERY_ITEMS: Record<string, UnknownRecord[]> = {
  ...
  M3_L2: [
    ...
    shortItem("M3L2_M6", "Why can a raised pod have energy while standing still?", ["because gravitational potential energy comes from position in a field", "because height store depends on being high not on moving", "because it has room to fall and transfer that store later", "because it has energy because of where it is in the gravitational field"], "Answer with position/field language.", phraseGroups(["position", "high", "height", "raised"], ["field", "gravity", "gravitational", "fall", "potential"])),
  ...
  M3_L3: [
    ...
    shortItem("M3L3_M6", "Why is speed more influential than mass in Motion Store comparisons?", ["because speed is squared while mass is linear", "because kinetic energy depends on v squared", "because doubling speed quadruples the store but doubling mass only doubles it"], "Use the squared-speed contrast explicitly.", phraseGroups(["speed", "v", "pace"], ["squared", "square", "quadruple", "four times", "v^2"], ["mass", "linear", "direct", "double", "doubles"])),
  ...
  M3_L5: [
    ...
    shortItem("M3L5_M4", "Why can two machines have the same power but different efficiency?", ["because they can transfer energy at the same rate but waste different amounts", "because rate and useful fraction are different ideas", "because the same power does not force the same useful yield"], "Use rate-versus-yield language.", phraseGroups(["same rate", "same power", "same transfer rate"], ["different waste", "different useful fraction", "different efficiency", "different yield", "different useful output"])),
  ...
  M3_L6: [
    ...
    shortItem("M3L6_M3", "Why should a long energy problem start with a mission plan instead of a formula guess?", ["because you need to know which step creates the next needed quantity", "because stores, leaks, and targets must be separated first", "because equation choice depends on the story sequence", "because one step provides the input needed by the next", "because later equations depend on earlier results"], "Use step-sequence and equation-choice language.", phraseGroups(["step", "sequence", "stage", "order", "plan"], ["next quantity", "store", "leak", "target", "bridge quantity", "result", "output", "input"], ["equation choice", "which equation", "formula", "equation", "depends", "feeds", "provides"])),
...
const M3_MASTERY_EXPANSIONS: Record<string, UnknownRecord[]> = {
  M3_L1: [
    ...
    shortItem("M3L1_M10", "Why does a balanced ledger matter in an energy mission?", ["because it tracks stores, hand-offs, and leaks across the whole mission", "because it forces energy to be accounted for step by step"], "Use ledger-accounting language.", phraseGroups(["track", "account", "ledger", "balance"], ["store", "hand-off", "transfer", "leak", "step", "mission"])),
  ],
  M3_L2: [
    ...
    shortItem("M3L2_M10", "Why is a height-only explanation weak in this lesson?", ["because Height Store also depends on mass and World Grip", "because mgh has three factors not one"], "Use all-three-factors language.", phraseGroups(["mass", "load", "m"], ["g", "world grip", "field", "gravity", "field strength"])),
  ],
  M3_L3: [
  ...
  M3_L6: [
    ...
    shortItem("M3L6_M10", "Why is equation order part of the physics in M3_L6?", ["because one stage creates the quantity needed by the next", "because stores, leaks, and targets determine which equation should come first", "because the output of one step becomes the input to the next", "because later equations need earlier results", "because each stage provides the bridge quantity for the next stage"], "Use sequence and bridge-quantity language.", phraseGroups(["stage", "next", "sequence", "order", "previous", "earlier", "later"], ["quantity", "store", "leak", "target", "bridge", "result", "output", "input"], ["equation", "order", "first", "choice", "depends", "feeds", "provides"])),
  ],
};
