...
const M3_DIAGNOSTIC_ITEMS: Record<string, UnknownRecord[]> = {
  M3_L1: [
    ...
    shortItem("M3L1_D7", "Why must the Energy Ledger balance?", ["because energy is accounted for in stores and leaks", "because energy is tracked in stores and leak trail", "because energy does not disappear but changes store or leaks"], "Answer in terms of energy being accounted for rather than disappearing.", phraseGroups(["accounted for", "balanced", "tracked"], ["store", "stores", "leak", "trail", "waste"], ["not disappear", "not destroyed", "does not vanish"])),
  ...
  M3_L4: [
    ...
    mcItem("M3L4_D8", "A learner pushes hard on a wall but the wall does not move. In the simple model, the work done on the wall is...", ["0 J", "the same as the force", "impossible to tell", "large because the effort is large"], 0, "No displacement means no work in this simple aligned-force story.", "The work done on the wall is 0 J because there is no displacement."),
...
const M3_MASTERY_ITEMS: Record<string, UnknownRecord[]> = {
  M3_L1: [
    ...
    mcItem("M3L1_M8", "Which sentence best keeps the module's hierarchy beyond F3?", ["Energy problems are ledger problems about stores, hand-offs, leaks, and targets", "Energy problems are mostly about memorizing one formula", "Energy is just the same as force with new units", "Only moving pods can matter in energy"], 0, "M3 treats energy as a linked system rather than a formula list.", "M3 pushes beyond a formula list by treating energy problems as ledger problems about stores, hand-offs, leaks, and targets."),
  ],
  M3_L2: [
  ...
  M3_L6: [
    ...
    mcItem("M3L6_M8", "Which statement best captures why M3 sits above the foundational energy module?", ["It expects store-ledger planning, deliberate equation choice, and multi-step mission accounting", "It only repeats the same single-step formula questions with new numbers", "It avoids qualitative reasoning and uses only substitution", "It treats efficiency and power as the same shortcut"], 0, "M3 is the richer planning-and-accounting version of the energy story.", "M3 is higher level because it expects store-ledger planning, deliberate equation choice, and multi-step mission accounting."),
  ],
};
