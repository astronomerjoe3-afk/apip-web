...
function questionVisualMeta(item: UnknownRecord): QuestionVisualMeta | undefined {
  ...
  const m13Visual = m13QuestionVisualMeta(normalizedId);
  ...
  const m9Visual = m9QuestionVisualMeta(normalizedId);
  ...
  const m12Visual = m12QuestionVisualMeta(normalizedId);
  if (m12Visual) return m12Visual;
...
function simulationStageTitle(code: string): string {
  const m13 = m13SimulationCopy(code);
  ...
  const m9 = m9SimulationCopy(code);
  ...
  const m12 = m12SimulationCopy(code);
  if (m12) return m12.title;
  switch (code) {
...
function simulationStageInstructions(code: string, inquiry: UnknownRecord[]): string {
  ...
  const m12 = m12SimulationCopy(code);
  if (m12) return m12.instructions;
  switch (code) {
...
function simulationStageExploreSteps(code: string): string[] {
  ...
  const m12 = m12SimulationCopy(code);
  if (m12) return m12.exploreSteps;
  switch (code) {
    case "F1_L1":
      return [
...
function scaffoldFocusExtras(code: string): string[] {
  const m13 = m13ScaffoldFocusExtras(code);
  ...
  const m9 = m9ScaffoldFocusExtras(code);
...
function scaffoldCoreBullets(code: string): string[] {
  const m13 = m13ScaffoldCoreBullets(code);
  if (m13.length > 0) return m13;
...
function scaffoldMediaCards(lesson: UnknownRecord): UnknownRecord[] {
  const code = lessonCode(lesson);
  const m13 = m13ScaffoldMediaCards(code);
  if (m13.length > 0) return m13;
  ...
  if (m5.length > 0) return m5;
  const m6 = m6ScaffoldMediaCards(code);
  if (m6.length > 0) return m6;
  const m8 = m8ScaffoldMediaCards(code);
  if (m8.length > 0) return m8;
  const m9 = m9ScaffoldMediaCards(code);
  if (m9.length > 0) return m9;
  const m10 = m10ScaffoldMediaCards(code);
  if (m10.length > 0) return m10;
  const m11 = m11ScaffoldMediaCards(code);
  if (m11.length > 0) return m11;
  const m12 = m12ScaffoldMediaCards(code);
  if (m12.length > 0) return m12;
...
function defaultAssessmentAlignmentContract(): UnknownRecord {
  return {
    exam_board: "Cambridge IGCSE",
    paper_structure: [
      { paper_id: "paper_1", format: "multiple_choice" },
      { paper_id: "paper_2", format: "structured" },
      { paper_id: "paper_4", format: "extended" },
      { paper_id: "paper_5", format: "practical_investigation" },
      { paper_id: "paper_6", format: "alternative_to_practical" },
    ],
...
function defaultCompetencyMappingContract(): UnknownRecord {
  return {
    metrics: [
      { key: "igcse_readiness_score", label: "IGCSE readiness score" },
      { key: "topic_mastery_index", label: "Topic mastery index" },
      { key: "practical_competency_rating", label: "Practical competency rating" },
      { key: "exam_simulation_performance", label: "Exam simulation performance" },
    ],
  };
}

function inferredSpiralFocus(code: string): UnknownRecord[] {
...
function defaultSpiralReinforcementContract(code: string): UnknownRecord {
  return {
    stages: [
      {
        level: "level_1",
    ...
    lesson_stage_focus: inferredSpiralFocus(code),
  };
}

function assessmentAlignmentContract(lesson: UnknownRecord): UnknownRecord {
  const authored = asRecord(asRecord(lesson.authoring_contract).assessment_alignment);
  const fallback = defaultAssessmentAlignmentContract();
  return {
    ...fallback,
    ...authored,
    paper_structure: asList(authored.paper_structure).length ? asList(authored.paper_structure) : asList(fallback.paper_structure),
    auto_generation_outputs: asList(authored.auto_generation_outputs).length
      ? asList(authored.auto_generation_outputs)
      : asList(fallback.auto_generation_outputs),
  };
}

function competencyMappingContract(lesson: UnknownRecord): UnknownRecord {
  const authored = asRecord(asRecord(lesson.authoring_contract).competency_mapping);
  const fallback = defaultCompetencyMappingContract();
  return {
    ...fallback,
    ...authored,
    metrics: asList(authored.metrics).length ? asList(authored.metrics) : asList(fallback.metrics),
  };
}

function spiralReinforcementContract(lesson: UnknownRecord): UnknownRecord {
  const code = lessonCode(lesson);
  const authored = asRecord(asRecord(lesson.authoring_contract).spiral_reinforcement);
  const fallback = defaultSpiralReinforcementContract(code);
  return {
    ...fallback,
    ...authored,
    stages: asList(authored.stages).length ? asList(authored.stages) : asList(fallback.stages),
    lesson_stage_focus: asList(authored.lesson_stage_focus).length
      ? asList(authored.lesson_stage_focus)
      : asList(fallback.lesson_stage_focus),
  };
}

function displayPaperId(paperId: string): string {
  const match = paperId.match(/paper_(\d+)/i);
  return match ? `Paper ${match[1]}` : paperId.replace(/_/g, " ");
}

function displayExamFormat(format: string): string {
  switch (format) {
    ...
    case "extended":
      return "extended questions";
    case "practical_investigation":
      return "practical investigation";
    case "alternative_to_practical":
...
function displayAutoGenerationOutput(output: string): string {
  switch (output) {
    case "timed_mock_exams":
      return "Timed mock exams";
    case "topic_level_past_paper_simulations":
      return "Topic-level past-paper simulations";
    case "weakness_heatmaps":
      return "Weakness heatmaps";
    default:
      return output.replace(/_/g, " ");
  }
}

function displaySpiralStage(stage: UnknownRecord): string {
  const level = text(stage.level).replace("level_", "Level ");
  const name = text(stage.stage).replace(/_/g, "/");
  return `${level} ${name}`;
}

function assessmentProgressionSection(lesson: UnknownRecord): UnknownRecord {
  const assessmentAlignment = assessmentAlignmentContract(lesson);
  const competencyMapping = competencyMappingContract(lesson);
  const spiralReinforcement = spiralReinforcementContract(lesson);
  const paperBullets = asList(assessmentAlignment.paper_structure)
    .map(asRecord)
    .map((paper) => {
      const paperId = displayPaperId(text(paper.paper_id));
      const format = displayExamFormat(text(paper.format) || text(paper.label));
      return paperId && format ? `${paperId}: ${format}.` : "";
    })
    .filter(Boolean);
  const autoGenerationOutputs = asList(assessmentAlignment.auto_generation_outputs)
    .map((entry) => displayAutoGenerationOutput(text(entry)))
    .filter(Boolean);
  const competencyBullets = asList(competencyMapping.metrics)
    .map(asRecord)
    .map((metric) => text(metric.label) || text(metric.key).replace(/_/g, " "))
    .filter(Boolean);
  const stages = asList(spiralReinforcement.stages).map(asRecord);
  const activeFocus = asRecord(asList(spiralReinforcement.lesson_stage_focus)[0]);
  const activeStageKey = `${text(activeFocus.level)}:${text(activeFocus.stage)}`;
  const stageBullets = stages.map((stage) => {
    const label = displaySpiralStage(stage);
    const detail = text(stage.description);
    const current = `${text(stage.level)}:${text(stage.stage)}` === activeStageKey;
    return current ? `${label}: this lesson's main emphasis. ${detail}` : `${label}: ${detail}`;
  });

  return {
    heading: "Assessment and progression",
    body: "This lesson feeds three visible course structures at once: Cambridge-style exam practice, competency tracking, and the spiral path from intuitive ideas to quantitative and analytical physics.",
    curriculum_panels: [
      {
        title: "Cambridge paper links",
        summary: autoGenerationOutputs.length
          ? `The platform also auto-builds ${autoGenerationOutputs.join(", ").toLowerCase()} from this paper structure.`
          : "These lessons are designed to connect with the full Cambridge paper mix, not just one question style.",
        bullets: paperBullets,
      },
      {
        title: "Progress tracking",
        summary: "Your performance is tracked as exam readiness, topic mastery, practical strength, and exam simulation strength so one weak area does not hide behind another.",
        bullets: competencyBullets,
      },
      {
        title: "Spiral stage in this lesson",
        summary: "The same concept returns three times across the curriculum: first for meaning, then for calculation, then for higher-level analysis.",
        bullets: stageBullets,
      },
    ],
    check_for_understanding: "Which paper style, progress metric, or spiral stage is this lesson strengthening most right now?",
  };
}

function withTechnicalWordsSection(lesson: UnknownRecord, sections: UnknownRecord[]): UnknownRecord[] {
  const insertSections = [technicalWordsSection(lesson), assessmentProgressionSection(lesson)].filter(
    (section): section is UnknownRecord => Boolean(section)
  );
  if (insertSections.length === 0) return sections;
  ...
  nextSections.splice(insertIndex, 0, ...insertSections);
  return nextSections;
}
...
function reflectionVisualCheck(lesson: UnknownRecord): UnknownRecord | undefined {
  const code = lessonCode(lesson);
  const m13 = m13ReflectionVisualCheck(code);
  if (m13) return m13;
  ...
  if (m5) return m5;
  const m6 = m6ReflectionVisualCheck(code);
  if (m6) return m6;
  const m8 = m8ReflectionVisualCheck(code);
  if (m8) return m8;
  const m9 = m9ReflectionVisualCheck(code);
  if (m9) return m9;
  const m10 = m10ReflectionVisualCheck(code);
  if (m10) return m10;
  const m11 = m11ReflectionVisualCheck(code);
  if (m11) return m11;
  const m12 = m12ReflectionVisualCheck(code);
  if (m12) return m12;
