...
export type M12QuestionVisualMeta = {
  image_url: string;
  visual_title: string;
...
const M12_ASSET_BASE = "/lesson_assets/M12";

const M12_VISUAL_META: Record<string, M12QuestionVisualMeta> = {
  M12L1: {
    image_url: `${M12_ASSET_BASE}/M12_L1/diagrams/m12-l1-field-weave-map.svg`,
  ...
  M12L2: {
    image_url: `${M12_ASSET_BASE}/M12_L2/diagrams/m12-l2-coil-tower.svg`,
  ...
  M12L3: {
    image_url: `${M12_ASSET_BASE}/M12_L3/diagrams/m12-l3-side-kick.svg`,
  ...
  M12L4: {
    image_url: `${M12_ASSET_BASE}/M12_L4/diagrams/m12-l4-spin-frame.svg`,
  ...
  M12L5: {
    image_url: `${M12_ASSET_BASE}/M12_L5/diagrams/m12-l5-change-thread.svg`,
  ...
  M12L6: {
    image_url: `${M12_ASSET_BASE}/M12_L6/diagrams/m12-l6-grid-bridge.svg`,
...
export function m12QuestionVisualMeta(itemId: string): M12QuestionVisualMeta | undefined {
  const match = itemId.toUpperCase().match(/^(M12L[1-6])_[A-Z]+\d+$/);
  return match ? M12_VISUAL_META[match[1]] : undefined;
}

const M12_SIMULATION_COPY: Record<string, M12SimulationCopy> = {
  M12_L1: {
...
export function m12SimulationCopy(code: string): M12SimulationCopy | undefined {
  return M12_SIMULATION_COPY[code];
}

export function m12ScaffoldFocusExtras(code: string): string[] {
  switch (code) {
    case "M12_L1":
...
export function m12ScaffoldCoreBullets(code: string): string[] {
  switch (code) {
    case "M12_L1":
...
export function m12ScaffoldMediaCards(code: string): UnknownRecord[] {
  const visual = M12_VISUAL_META[code.replace("_", "")];
  if (!visual) return [];
  return [
    {
      kind: "visual",
...
export function m12ReflectionVisualCheck(code: string): UnknownRecord | undefined {
  const visual = M12_VISUAL_META[code.replace("_", "")];
