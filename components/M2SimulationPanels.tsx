...
export default function M2SimulationPanels({
  ...
  if (lessonKey === "M2_L5") {
    ...
    const loadWidthPx = 84;
    const loadHeightPx = 44 + ((loadHeight - 1) / 7) * 44;
    const loadLeft = lineX - loadWidthPx / 2;
    const loadTop = supportBarY - loadHeightPx - 16;
    const balanceCoreY = loadTop + 22;
    ...
    return render(
      ...
      <svg viewBox="0 0 640 250" className="w-full">
        <rect x="12" y="12" width="616" height="226" rx="24" fill="#f8fafc" />
        <text x="36" y="42" fill="#0f172a" fontSize="20" fontWeight="700">Stability comes from where the weight line lands</text>
        <text x="36" y="66" fill="#475569" fontSize="15">{stableNow ? "Balance Core line still lands inside the support zone." : "Balance Core line has crossed the support edge."}</text>
        <circle cx="46" cy="92" r="7" fill="#0ea5e9" />
        <text x="60" y="97" fill="#0369a1" fontSize="14" fontWeight="700">Balance Core line</text>
        ...
        <text x={centerX} y={supportBarY + 34} fill="#475569" fontSize="14" textAnchor="middle">Footprint Zone</text>
        <line x1={lineX} y1="214" x2={nearestEdgeX} y2="214" stroke={stableNow ? "#16a34a" : "#dc2626"} strokeWidth="4" strokeLinecap="round" />
        <polygon points={`${lineX},214 ${lineX + (cargoOffset >= 0 ? 10 : -10)},208 ${lineX + (cargoOffset >= 0 ? 10 : -10)},220`} fill={stableNow ? "#16a34a" : "#dc2626"} />
        <polygon points={`${nearestEdgeX},214 ${nearestEdgeX + (cargoOffset >= 0 ? -10 : 10)},208 ${nearestEdgeX + (cargoOffset >= 0 ? -10 : 10)},220`} fill={stableNow ? "#16a34a" : "#dc2626"} />
        <text x={(lineX + nearestEdgeX) / 2} y="206" fill={stableNow ? "#166534" : "#b91c1c"} fontSize="14" fontWeight="700" textAnchor="middle">{marginLabel}</text>
        <text x="36" y="236" fill="#475569" fontSize="15">{stableNow ? "Wider support or smaller offset leaves more room before tipping." : "The load line has moved beyond the base edge, so tipping begins."}</text>
      </svg>,
      <>
