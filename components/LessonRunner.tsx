...
type Props = {
  ...
  // Optional: allow parent to navigate to next lesson cleanly (future)
  onRequestNextLesson?: () => void;
};

type McqItem = {
  type: "mcq";
  question_id?: string; // hidden from student UI
  prompt: string;
  choices: string[];
  correct_index?: number; // seeded content (internal scoring)
...
// A few helpful default hints for short answers (most written tasks).
// We DO NOT show tags to students; we only use tags to choose a hint.
...
function pickHint(item: any): string {
  ...
  // generic fallback hint for written tasks
...
function scoreMcq(items: Item[], chosenMap: Record<string, number>) {
...
export default function LessonRunner({ moduleId, lesson, misconceptionAllowlist, onRequestNextLesson }: Props) {
  ...
  // Map seeded phases into ACRM student-facing sequence:
  // 1) Prediction Gate  <- diagnostic
  // 2) Analogy (MIM)    <- analogical_grounding (+ a simple tool-trust choice)
  // 3) Virtual Lab      <- simulation_inquiry (if lab_id)
  // 4) Build Concepts   <- concept_reconstruction (explanations + written prompts + hints)
  // 5) Challenge        <- transfer (scored)
  //
  // NOTE: No question appears twice: each item list used exactly once.
  ...
  // Simple Analogy (MIM) tool-trust interaction
  ...
  // mastery (student-visible) = derived from highest scores in formative+summative activities (best per lesson)
  const [bestScores, setBestScores] = useState<BestScores>({});
  const moduleMastery = useMemo(() => computeModuleMastery(bestScores), [bestScores]);

  // backend progress (kept hidden; used only to sanity-check / future instructor tools)
  const [progress, setProgress] = useState<ProgressMe | null>(null);

  useEffect(() => {
    // load best scores when module changes
    if (typeof window !== "undefined") {
      setBestScores(readBestScores(moduleId));
    }
  }, [moduleId]);
  ...
  const studentOnlyTagsFor = (items: Item[]) => {
    // NOT shown to student; only passed to backend (filtered allowlist)
    const tags: string[] = [];
  ...
  async function submitPredictionGate() {
    ...
    if (hasShort) {
      const missing = (diagnosticItems || [])
        ...
        .some((q: any) => !(short[q.question_id || ""] || "").trim());
      if (missing) {
    ...
    persistBestScore(score); // keep highest score only (client-side)
    ...
    setFeedback({
      ...
      body:
        score >= PASS_THRESHOLD
          ? "You can continue."
          : "You can continue now. If you want, you can repeat later to improve your mastery.",
    });
  ...
  async function submitAnalogy() {
    // Require tool-trust choice + short explanation (hinted)
    if (!toolChoice) {
  ...
  async function submitLabOnce() {
    // Virtual lab runs once (no repeats)
    // Require at least one written observation to ensure engagement (hinted)
  ...
  async function submitBuildConcepts() {
    // Build concepts: require each reconstruction prompt answered (written tasks get hints)
  ...
  async function submitChallenge() {
    // Summative score = MCQ correctness from transferItems
      ...
      <div style={styles.card}>
            ...
            <div style={styles.questionCard}>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <div style={styles.hint}>
                  Hint: the more precise tool gives more reliable measurements.
                </div>
              </div>
          ...
          <>
            ...
            {simLabId ? (
              <p style={styles.bodyText}>
                Do the lab once, then write one observation.
              </p>
            ) : null}
        ...
        {step === "build" ? (
          <>
            ...
            <p style={styles.bodyText}>
              Answer these short prompts to make the concept clear before the final challenge.
            </p>
  ...
  return (
    <div style={{ display: "grid", gap: 12, marginTop: 10 }}>
      {items.map((q: any, idx: number) => {
        ...
        const qid = String(q?.question_id || `q_${idx}`); // internal key only; NOT shown
            ...
            {q.type === "mcq" ? (
              <div style={{ display: "grid", gap: 10 }}>
                {(q.choices || []).map((c: string, cidx: number) => {
                  const active = mcq[qid] === cidx;
                  return (
                    <button
                      key={cidx}
                      onClick={() => setMcq({ ...mcq, [qid]: cidx })}
                      style={{
                        textAlign: "left",
                        padding: "14px 14px",
                        borderRadius: 14,
              ...
              <>
                <textarea
                  ...
                  value={short[qid] || ""}
                  onChange={(e) => setShort({ ...short, [qid]: e.target.value })}
